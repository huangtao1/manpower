...
from opts import db, login_manager
...
class User(UserMixin, db.Model):
    ...
    用户表
    """
    __tablename__ = 'opts_user'
    id = db.Column(db.Integer, primary_key=True)
    email = db.Column(db.String(64), unique=True, index=True)
    username = db.Column(db.String(128), unique=True, index=True)
    display_name = db.Column(db.String(64), nullable=True)
    role_id = db.Column(db.Integer, db.ForeignKey('opts_role.id'))
    role = db.relationship('Role', backref=db.backref('user', order_by=id))
    organize_id = db.Column(db.Integer, db.ForeignKey('opts_organization.id'))
...
class Menu(db.Model):
    ...
    parent_id = db.Column(db.Integer, db.ForeignKey('opts_menu.id'), nullable=True)
...
class Organization(db.Model):
    ...
    组织架构表
    """
    __tablename__ = 'opts_organization'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), unique=True)
    is_parent = db.Column(db.Boolean, default=False)
    parent_id = db.Column(db.Integer, db.ForeignKey('opts_organization.id'), nullable=True)
